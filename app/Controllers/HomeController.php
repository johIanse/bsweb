...
class HomeController{public function home(){
    ...
    try{$p=Database::pdo();$stats['totalUsers']=(int)$p->query('SELECT COUNT(*) FROM users')->fetchColumn();$stats['activeUsers']=(int)$p->query('SELECT COUNT(*) FROM users WHERE status=1')->fetchColumn();$st=$p->prepare("SELECT COUNT(*) FROM step_logs WHERE status='success' AND steps IS NOT NULL AND substr(created_at,1,10)=?");$st->execute([date('Y-m-d')]);$stats['todaySuccess']=(int)$st->fetchColumn();$start=(string)$p->query("SELECT MIN(created_at) FROM users")->fetchColumn();if($start){$stats['startDate']=substr($start,0,10);$stats['runDays']=max(1,(int)floor((time()-strtotime($start))/86400)+1);}}catch(\Throwable $e){}
