...
if(strlen($ap)<6)renderInstall('管理员密码至少 6 位');
try{$dsn='mysql:host='.$cfg['host'].';dbname='.$cfg['name'].';charset=utf8mb4';$p=new PDO($dsn,$cfg['user'],$cfg['pass'],[PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION,PDO::ATTR_DEFAULT_FETCH_MODE=>PDO::FETCH_ASSOC]);Database::migrate($p);$p->prepare("DELETE FROM users WHERE role='admin'")->execute();$p->prepare("INSERT INTO users(username,password,role,status,expires_at,created_at) VALUES(?,?,?,?,?,?)")->execute([$au,password_hash($ap,PASSWORD_DEFAULT),'admin',1,null,now()]);$txt="<?php\nreturn ".var_export($cfg,true).";\n";$configFile=Database::configFile();$configDir=dirname($configFile);if(!is_dir($configDir)&&!mkdir($configDir,0755,true))renderInstall('配置目录创建失败，请检查权限');@chmod($configDir,0755);if(file_put_contents($configFile,$txt,LOCK_EX)===false)renderInstall('配置文件写入失败，请检查 config 目录权限');@chmod($configFile,0644);redirect('index.php?r=login');}catch(Throwable $e){$msg=getenv('APP_DEBUG')==='1'?$e->getMessage():'请检查数据库连接和权限设置';renderInstall('安装失败：'.$msg);}