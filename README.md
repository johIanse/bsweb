...
```bash
curl -fsSL https://raw.githubusercontent.com/johIanse/bsweb/main/install-online.sh | sudo bash -s -- --single --admin-user 上传 --admin-pass admin123
```
...
```bash
curl -fsSL https://raw.githubusercontent.com/johIanse/bsweb/main/install-online.sh | sudo bash -s -- --reset-admin --admin-user 上传 --admin-pass admin123
```
